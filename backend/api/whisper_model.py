...
async def transcribe_and_send(transcript_parts: list, file_path: str, send_message_threadsafe: callable):
    try:
        ...
        transcript_parts.append(transcription) # store transcript parts for final assembly
...
async def flush_remaining_buffer(transcript_parts: list, buffer: bytearray, webm_header: bytes, send_message_threadsafe: callable):
    if len(buffer) > 0:
        ...
        await transcribe_and_send(transcript_parts, wav_path, send_message_threadsafe)
